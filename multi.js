...
const Multi = (()=>{
  ...
  async function sendPos(){
    ...
    try{
      ...
      arr.push({ id:pid(), name:(G.save.name||'Angler').slice(0,16), color:myColor(),
        ...
        head:Math.round(G.boat.head*100)/100, boat:save.boat, title:(activeTitle()||''), badges:{betaTester:!!(save.badges&&save.badges.betaTester),daveTest:!!(save.badges&&save.badges.daveTest),creator:!!(save.badges&&save.badges.creator),pinkfong:!!(save.badges&&save.badges.pinkfong),witchy:!!(save.badges&&save.badges.witchy)}, at:now });
      ...
      for(const p of arr){
        ...
        if(!pl){
          pl={ id:p.id, name:p.name||'Angler', color:p.color||colorOf(p.id), lvl:p.lvl||1, boat:p.boat||'surf',
            title:p.title||'', badges:p.badges||{betaTester:!!p.beta,daveTest:false,creator:false,pinkfong:false,witchy:false}, x:+(p.x||0), y:+(p.y||0), head:+(p.head||0),
        ...
        pl.title=p.title||pl.title; pl.badges=p.badges||{betaTester:!!p.beta,daveTest:false,creator:false,pinkfong:false,witchy:false}; pl.color=p.color||pl.color; pl.at=now;
      }
  ...
  function drawTag(sx,sy,name,lvl,color,chat,title,badges){
    ...
    if(title){
      ...
      ctx.font=(isCreator?'900 11px':'700 7.5px')+' system-ui,sans-serif';
      const tw=ctx.measureText(title).width+(isCreator?22:13);
      const th=isCreator?18:13, tx=sx-tw/2, ty=sy-(isCreator?92:85);
      const titleMeta=TITLES.find(t=>t.name===title);
      const titleColor=(titleMeta&&titleMeta.color)||'#ffd166';
      ctx.fillStyle=isCreator?'rgba(63,91,117,.95)':title==='PINKFONG!'?'rgba(86,17,74,.94)':title==='WITCHY'?'rgba(12,10,23,.94)':'rgba(12,8,0,.92)';
      roundRect(tx,ty,tw,th,th/2); ctx.fill();
      ctx.strokeStyle=titleColor; ctx.lineWidth=isCreator?1.6:1; ctx.stroke();
      ctx.fillStyle=titleColor;
      if(isCreator){ctx.shadowColor='#fff';ctx.shadowBlur=8;} ctx.fillText(title, sx, ty+th/2); ctx.shadowBlur=0;
    }
    ...
    if(badges.creator){
      ...
      ctx.fillStyle='#f6fbff'; ctx.strokeStyle='#fff'; ctx.lineWidth=1;
      ctx.shadowColor='#ffd769'; ctx.shadowBlur=7; ctx.beginPath(); ctx.arc(bx,y+2,6,0,Math.PI*2); ctx.fill(); ctx.stroke(); ctx.shadowBlur=0;
      ctx.fillStyle='#b27d08'; ctx.font='900 9px system-ui,sans-serif'; ctx.fillText('✧',bx,y+2.2);
    }
    if(badges.pinkfong){
      const bx=x+w-2+(badges.betaTester?14:0)+(badges.daveTest?14:0)+(badges.creator?14:0);
    ...
    if(badges.witchy){
      const bx=x+w-2+(badges.betaTester?14:0)+(badges.daveTest?14:0)+(badges.creator?14:0)+(badges.pinkfong?14:0);
